...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('icto-theme');
  ...
  function toggle() {
    ...
    apply(next);
    setDark(next);
    localStorage.setItem('icto-theme', next ? 'dark' : 'light');
  }
  ...
  return (
    <button
      ...
      className="
        ...
        dark:bg-slate-800 dark:border-slate-700 dark:text-cyan-400 dark:hover:bg-slate-700
      "
    >
      {dark ? (
